...
| `traj_opt/schedule.py` | phase table — flight **50 knots** (26→50 on 2026-09-04; 99 tried 2026-09-05 and reverted, 9/11) | — |
...
| `traj_opt/warm_start.py` | mesh-refinement warm-start: export a solved trajectory resampled at a new knot count, `--warm-start`/`--start-checkpoint` on `solve_backflip.py` | **working as of 2026-09-05, and it never was before** — it round-trips the shipped point bit-exactly (0.128651, 10/11). The three re-timing bugs are described in that session's section. The old note here said it was "the wrong tool for 14→20, resampling an under-resolved source"; that was a misdiagnosis, the source's resolution was never the problem. Refine on a **2n-1** grid so the new knots are a superset of the old, and prefer `--states shoot` (the default) |
